...
def calculate_linear_regression(stock_return_df, consumption_return_df):
    ...
    After, the regression in form of statsmodels.regression is stored inside a dicitionary inside the key named the same as
    the ticker.
    ...
    for stock in cols_retorno_stocks:
        start_quarter = stock_return_df[stock_return_df[stock].notna()].index[0] # getting where the price serie start
        slice_stock_return_df = stock_return_df[[stock,'Constant','Consumption']]
        slice_stock_return_df = slice_stock_return_df.loc[start_quarter:,:] # slicing to get where the series start
        slice_stock_return_df.fillna(0, inplace=True) # changin remain nan to zero, assuming there were no negotiation

        reg_ = sm.OLS(endog=slice_stock_return_df[stock], exog=slice_stock_return_df[['Constant','Consumption']],missing=0).fit()
        reg_dict[stock] = reg_
...
def generate_regression_dataframe(reg_dict):
    ...
    lst_columns = ['coef_constante', 'std_err_constante', 'p_value_constante', 'coef_consumption', 'std_err_consumption',
                  'p_value_consumption', 'r_squared', 'r_squared_adj', 'f_stats', 'p_value_f_stats', 'durb_watson']
